...
def shorten_contract(input_contract: str):
    ic()
    new_contract = ""
    ...
    if len(new_contract) > 15000:
        return shorten_contract(new_contract)
    
    return new_contract

def extract_card_details(input_contract: str):

    if len(input_contract) > 20000:
        input_contract = shorten_contract(input_contract)
...
if __name__ == "__main__":
    with open("../data/Apple-Card-Customer-Agreement.txt") as infile:
        apple_snippet_data = infile.read()
        print(apple_snippet_data)
    extract_card_details(apple_snippet_data)
